  ...
  useEffect(() => {
    const fetchReadLaterPapers = async () => {
      ...
      try {
        ...
        toast.error('Failed to load read later papers');
      } finally {
        setLoading(false);
      }
    };

    fetchReadLaterPapers();
  }, [user]);
        ...
        >
          <div className="glass-card p-6 md:p-8 mb-8">
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2 flex items-center gap-2">
              ...
              Your Read Later Papers
            </h1>
            <p className="text-slate-600">
              Papers you've saved to read later
            </p>
          </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="glass-card p-8 text-center">
                ...
                <p className="text-slate-600">Loading your read later papers...</p>
              </div>
            </div>
          ) : papers.length === 0 ? (
            <div className="text-center py-12">
              <div className="glass-card p-8 max-w-md mx-auto">
                ...
                <h3 className="text-lg font-semibold text-slate-800 mb-2">
                  No Read Later Papers Yet
                </h3>
                <p className="text-slate-600">
                  Save papers to read later to see them here!
                </p>
              </div>
            </div>
          ) : (
