...
export const getYesterdaysPapers = async (topic: string, maxResults: number = 6): Promise<ArXivPaper[]> => {
  const yesterday = new Date();
  yesterday.setDate(yesterday.getDate() - 4);
  const dateStr = yesterday.toISOString().split('T')[0].replace(/-/g, '');
  
  const params: Record<string, string> = {
    search_query: `all:${topic} AND submittedDate:[${dateStr}000000 TO ${dateStr}235959]`,
    start: '0',
